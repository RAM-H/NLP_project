...
def main():
    ...
    trained_nlp = spacy.load("models/output_descriptors/model-best")
    if choice == 'Descripteures':
        ...
        trained_nlp = spacy.load("models/output_species/model-best")
        st.subheader("Named Entity Recog with Spacy")
        raw_text = st.text_area("Enter Text Here","Type Here")
        if st.button("Analyze"):
            docx = trained_nlp(TOKENIZE(raw_text.lower()))
        ...
        trained_nlp = spacy.load("models/output_mixte/model-best")
        st.subheader("Named Entity Recog with Spacy")
        raw_text = st.text_area("Enter Text Here","Type Here")
        if st.button("Analyze"):
            docx = trained_nlp(TOKENIZE(raw_text.lower()))
